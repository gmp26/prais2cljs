  ...
  [{:section "Background"
    ...
    [{:title "Why is a different survival range predicted for each hospital?"
     ...
     {:title "When looking at just one hospital, what does it mean if its survival is outside its predicted range?"
      ...
      [:div
       ...
       Although the data submitted is of very high quality (MIKE LINK TO FAQ on DATA QUAL), there will always be some
       ...
       range of survival. Although the formula as is as good as we can currently get it, it is not perfect
       (MIKE LINK TO FAQ on FORM QUAL (1st one)). There will always be unique features about a child that affect their
   ...
   {:section "My family or child"
    ...
    [{:title "Which hospital should I go to?"
      ...
      :body
      [:div
       [:p "You can use the national audit data to see how the different hospitals are doing compared to their predicted
       range calculated for the last 3 years (MIKE LINK TO EXPLORE THE DATA).
       You can also use the (MIKE LINK change - see word doc) "
        [:a {:href "https://nicor4.nicor.org.uk/CHD/an_paeds.nsf/vwContent/home" :target "_blank"} "national audit website"]
      ...
      [:div
       ...
       [:.image-para
        [:p [:img.pull-left {:style {:width "150px" :margin-right "20px"} :src "assets/camlogo.png"}]
         ...
         predicted range.  "
         [:strong "Mike Pearson"] ", University of Cambridge,  is the brains behind the website development, setting up
         the site, the data visuals and working closely with the animation team, Qudos. (MIKE LINK to Qudos)."]]
