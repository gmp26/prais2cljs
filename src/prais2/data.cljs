...
(rum/defc hospital-charities < rum/reactive []
  (let [ap (rum/react core/app)
    ...
    [:div
     [:h4 "Further web information"]
     [:ul
      [:li [:a (link1 1) (link1 2)]]
  ...
  (let [ap (rum/react core/app)
    ...
    (if h-code
      ...
      (let [selected-row content/sample-hospital]
        [:#detail
         [:i
          [:p "Below is a chart showing how we present the results of a sample hospital."]
          [:p "Mouse over or click on the chart bars and the dot for explanations of their meaning."]
          [:p "Now use the map menu or click on a hospital location to see the real results and links to further information."]]
         [:h3 (:h-name selected-row)]
         (slider-widget content/header-row detail-slider-control (:detail-slider-axis-value ap))
         (chart-cell selected-row (:detail-slider-axis-value ap))
         (interpretation selected-row)]))))
