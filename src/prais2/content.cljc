...
(defn ->map
  "If m is a map, returns m, if it is a string, returns the underlying map.
   It is very useful because of how differently webservers can interpret edn files (out as a string or the underlying datastructure)"
  [m]
  (cond
...
(defn get-file
  ...
  [endpoint]
  (go
    (let [data #?(:clj (clojure.edn/read-string (slurp endpoint))
                  :cljs (:body (<! (http/get endpoint {:as :auto}))))]
      (->records data))))

(defn get-edn-file
  "Loads edn data from target/public without Row transformations"
  [endpoint]
  (go
    (let [data #?(:clj (clojure.edn/read-string (slurp (str "target/public/" endpoint)))
                  :cljs (:body (<! (http/get endpoint {:as :auto}))))]
      (->map data))))
...
(comment
  ...
  #_[(Row. "Newcastle, Freeman Hospital" "FRE" 55.002386 -1.593643                         694	11	683	  98.4	97.5	95.5	96.3	98.6	99.1 nil)
  ...
  #_[(Row. "Newcastle, Freeman Hospital" "FRE" 55.002386 -1.593643                         694	11	683	  98.4	95.5	96.3	98.6	99.1 nil)
...
(defn get-hospital-metadata
  []
  (take!
   (get-edn-file "/hospital-metadata.edn")
   #(do
      (swap! core/app assoc :hosp-metadata %))))

#_(def hospital-metadata
...
(defn sorted-active-h-codes [data]
  (map first (sort-by second (map (fn [hosp-meta] [(keyword (:h-code hosp-meta)), (:h-name hosp-meta)]) data))))

(defn get-unassoc-charities
  []
  (take!
   (get-edn-file "/unassoc-charities.edn")
   #(do
      (swap! core/app assoc :unassoc-charities %))))

#_(def unassoc-charity-list
...
(rum/defc render-charity-list < rum/static rum/reactive []
  [:div
   [:h3 "Charities"]
   [:ul.charities
    (for [charity #_unassoc-charity-list (:unassoc-charities (rum/react core/app))]
      [:li {:key (#?(:cljs random-uuid :clj java.util.UUID/randomUUID))} charity])]
   [:h3 "Hospitals and hospital-associated charities"]
   [:ul.charities
    (for [h-code (sorted-active-h-codes ((:datasource (rum/react core/app)) (:hosp-data (rum/react core/app))))]
      [:li {:key (#?(:cljs random-uuid :clj java.util.UUID/randomUUID))} [:b (first (h-code (:hosp-metadata (rum/react core/app))))]
       [:ul
        (for [charity (rest (h-code (:hosp-metadata (rum/react core/app))))]
          [:li {:key (#?(:cljs random-uuid :clj java.util.UUID/randomUUID))} charity])]])]])
