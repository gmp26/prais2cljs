(ns ^:figwheel-always prais2.main
  ...
  (:require [rum.core]
            ...
            [cljsjs.react]
            [cljs.core.async :refer [chan <! pub sub put! close! take!]]
            ...
            [prais2.load-edn :refer [make-remote-call]]
...
(rum.core/defc app-container < bs-popover bs-tooltip []
  (take! (make-remote-call "http://localhost:3449/data.edn")
         #(js/console.log %))
  (render-page)
  )
...
(if-let [mount-point (core/el "app")]
  (do
    ;; (swap! core/app #(assoc % :hosp-data {:2019 "test"}))
    ;; (take! (make-remote-call "http://localhost:3449/data.edn")
    ;;        #(do (swap! core/app assoc :hosp-data %)
    ;;             (js/console.log (str "in state callback: " %))))
    ;; (js/console.log (str "in state: " (:hosp-data @app)))
